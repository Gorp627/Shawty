...
// --- Configuration ---
const CONFIG = { PLAYER_DEFAULT_HEALTH: 100, PLAYER_RESPAWN_DELAY: 3500, SERVER_BROADCAST_INTERVAL: 1000 / 15, PLAYER_MOVE_THRESHOLD_SQ: 0.0001, MAX_PLAYERS: 20 };
...
io.on('connection', function(socket) {
    ...
    socket.on('hit',function(d){console.log(">>> Received 'hit'",d);const t=players[d.targetId],s=players[socket.id];if(t&&t.health>0&&s&&socket.id!==d.targetId){const died=t.takeDamage(d.damage||10,s);if(died){console.log(`--- Emitting 'playerDied' for ${t.id}`);io.emit('playerDied',{targetId:d.targetId,killerId:s.id,killerName:s.name,killerPhrase:s.phrase});}else{console.log(`--- Emitting 'healthUpdate' for ${t.id}`);io.emit('healthUpdate',{id:d.targetId,health:t.health});}}else{console.warn(`Hit ignored: Tgt=${!!t}, TgtAlive=${t?.health>0}, Shtr=${!!s}, SelfHit=${socket.id===d.targetId}`);}});
...
let lastBroadcastTime = 0; function serverGameLoop() { let stateUpdate={players:{}};let uGen=false; for(const id in players){if(players[id].needsUpdate){stateUpdate.players[id]=players[id].getNetworkData();players[id].needsUpdate=false;uGen=true;}} if(uGen){io.emit('gameStateUpdate',stateUpdate);lastBroadcastTime=Date.now();} } const gameLoopIntervalId=setInterval(serverGameLoop,CONFIG.SERVER_BROADCAST_INTERVAL);
// --- HTTP Server ---
