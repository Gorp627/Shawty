...
io.on('connection', function(socket) {
    ...
    socket.on('hit',function(d){console.log(">>> Received 'hit'",d);const t=players[d.targetId],s=players[socket.id];if(t&&t.health>0&&s&&socket.id!==d.targetId){const died=t.takeDamage(d.damage||10,s);if(died){console.log(`--- Emit DIE ${t.id}`);io.emit('playerDied',{targetId:d.targetId,killerId:s.id,killerName:s.name,killerPhrase:s.phrase});}else{console.log(`--- Emit HP ${t.id}`);io.emit('healthUpdate',{id:d.targetId,health:t.health});}}else{console.warn(`Hit ignored: Tgt=${!!t}, TgtAlive=${t?.health>0}, Shtr=${!!s}, Self=${socket.id===d.targetId}`);}});
