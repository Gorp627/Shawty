...
io.on('connection', function(socket) { // Using function() syntax
    ...
    socket.on('setPlayerDetails', function(details) { // Using function() syntax
        ...
        if (players[socket.id]) { players[socket.id].name=finalName; players[socket.id].phrase=finalPhrase; return; } // Allow update?
...
function scheduleRespawn(playerId) { setTimeout(function() { const p = players[playerId]; if (p) { p.health = 100; p.x = Math.random() * 10 - 5; p.y = 0; p.z = Math.random() * 10 - 5; p.rotationY = 0; console.log(`${p.name} respawned.`); io.emit('playerRespawned', p); } }, RESPAWN_DELAY); } // Send full data including name/phrase
// --- Basic HTTP Server ---
