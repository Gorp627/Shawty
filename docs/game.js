...
class Game {
    ...
    attemptProceedToGame() {
        ...
        } else { console.log(`[Game] Prerequisites not met or invalid state. State: ${stateMachine?.currentState}`); }
    }
        ...
        console.log("[Game] Init Core Components...");
         try { // Three.js ...
             ...
             const playerPlayerContact = new CANNON.ContactMaterial(playerMaterial, playerMaterial, {
                 friction: 0.5,      // Higher friction between players
                 restitution: 0.2    // Some bounciness
             });
             this.world.addContactMaterial(playerPlayerContact); // <<< ADDED PLAYER-PLAYER CONTACT
             console.log("[Game] Defined physics materials and contacts.");

             // -- Create Ground Physics Body --
             const groundShape = new CANNON.Plane();
             groundBody = new CANNON.Body({ mass: 0, shape: groundShape, material: groundMaterial }); // Assign to global groundBody ref
    ...
    initializeManagers() {
        console.log("[Game] Init Managers...");
         if(typeof UIManager === 'undefined' || typeof Input === 'undefined' || typeof stateMachine === 'undefined' || typeof loadManager === 'undefined' || typeof Network === 'undefined' || typeof Effects === 'undefined') { /* ... error check ... */ return false; }
    ...
    bindOtherStateTransitions() {
        ...
        if (stateMachine) {
            stateMachine.on('transition', (data) => {
                 console.log(`[Game Listener] Transition EVENT: ${data.from} -> ${data.to}`);
    ...
    handlePlayerGroundContact(event) {
        // Check if the collision involves the local player's body and the ground body
        const isPlayerCollision = (event.bodyA.id === this.physicsBodies[localPlayerId]?.id || event.bodyB.id === this.physicsBodies[localPlayerId]?.id);
        const isGroundCollision = (event.bodyA === groundBody || event.bodyB === groundBody);

        if (isPlayerCollision && isGroundCollision) {
            isPlayerGrounded = true; // Set the global flag
             // Optional: console.log("Ground contact detected!");
        }
    }
        ...
        for(const id in initData.players){
            ...
            try {
                if(id === localPlayerId){ // --- LOCAL PLAYER ---
                    ...
                    if (!playerBody) throw new Error("Local body creation failed."); playerBody.angularFactor.set(0,1,0);

                    // <<< ADD COLLISION EVENT LISTENER >>>
                    playerBody.addEventListener('collide', this.handlePlayerGroundContact.bind(this)); // Use bind to keep 'this' context
                    console.log(`[Game] Added collision listener to local player body.`);
                    // <<< END LISTENER >>>
                    ...
                    if(UIManager){ UIManager.updateHealthBar(sPD.health ?? 100); UIManager.updateInfo(`Playing as ${players[id].name}`); UIManager.clearError('homescreen'); UIManager.clearKillMessage(); }
                     ...
                     if (remotePlayer instanceof ClientPlayer && world) {
                         ...
                         if (!remoteBody) throw new Error(`Remote body ${id} creation failed.`); const initialRotationY = sPD.rotationY || 0; remoteBody.quaternion.setFromEuler(0, initialRotationY, 0);
...
console.log("game.js loaded (Added Collision Listener)");
