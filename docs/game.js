...
class Game {
    ...
    start() {
        ...
        this.mapMesh = null; this.physicsBodies = {}; this.world = null; this.lastCallTime = performance.now();
    ...
    initializeNetwork() {
        ...
        if (typeof Network !== 'undefined' && typeof Network.init === 'function') { /* ... Network init logic ... */ return true; }
        else { console.error("Network missing/invalid"); stateMachine?.transitionTo('loading', {message: "Network Load Failed", error:true}); return false; }
    }
    ...
    bindLoadManagerListeners() {
        ...
        if (typeof loadManager === 'undefined') { /* ... error handling ... */ return; }
        loadManager.on('ready', () => {
            ...
            if (!this.mapMesh) { console.error("Map asset data missing!"); stateMachine?.transitionTo('loading', { message: "Map Data Load Failed!", error: true }); return; }
    ...
    attemptProceedToGame() {
        ...
        if (assetsAreReady && networkIsInitialized && initializationData) {
            console.log("[Game] Prerequisites met. Starting game play...");
    ...
    initializeCoreComponents() {
        console.log("[Game] Init Core Components...");
         try { // Three.js ... scene, camera, renderer, controls, loaders, lights ...
             ...
             const groundShape = new CANNON.Plane(); const groundBody = new CANNON.Body({ mass: 0, shape: groundShape, material: groundMaterial }); groundBody.quaternion.setFromAxisAngle(new CANNON.Vec3(1, 0, 0), -Math.PI / 2); groundBody.position.set(0, 0, 0); this.world.addBody(groundBody); console.log("[Game] Cannon.js World OK."); return true;
    ...
    initializeManagers() {
        console.log("[Game] Init Managers...");
         ...
         } catch (e) { console.error("!!! Mgr Init Error:", e); UIManager?.showError("FATAL: Mgr Setup Error!", 'loading'); return false; }
    }
    ...
    bindOtherStateTransitions() {
        ...
        if (stateMachine) {
            stateMachine.on('transition', (data) => {
                 console.log(`[Game Listener] Transition: ${data.from} -> ${data.to}`);
                 if (data.to === 'homescreen') {
                     ...
                     if (data.from === 'playing' || data.from === 'joining') {
                         console.log(`Cleanup after ${data.from}...`); for(const id in this.physicsBodies) { if (this.world) this.world.removeBody(this.physicsBodies[id]); } this.physicsBodies = {};
                 ...
                 } else if (data.to === 'playing') { /* UI updates */ }
    ...
    // --- Main Update/Animate Loop ---
     animate(physicsTimeStep) {
        ...
        if(stateMachine?.is('playing')){ try{ const localPlayerBody = localPlayerId ? this.physicsBodies[localPlayerId] : null; if (updateLocalPlayer) updateLocalPlayer(dt, localPlayerBody); } catch(e){console.error("Err updateLP:",e);} try{ if(Effects?.update) Effects.update(dt); } catch(e){console.error("Err Effects.update:",e);}
             const localBody = localPlayerId ? this.physicsBodies[localPlayerId] : null; if (localBody && controls?.getObject()) { controls.getObject().position.copy(localBody.position); controls.getObject().position.y += (CONFIG?.CAMERA_Y_OFFSET ?? 1.6); }
             for (const id in players) { if (id !== localPlayerId && players[id] instanceof ClientPlayer && players[id].mesh) { const remoteBody = this.physicsBodies[id]; if (remoteBody) { players[id].mesh.position.copy(remoteBody.position); players[id].mesh.quaternion.copy(remoteBody.quaternion); const playerHeight = CONFIG?.PLAYER_HEIGHT || 1.8; if (!(players[id].mesh.geometry instanceof THREE.CylinderGeometry)) { players[id].mesh.position.y -= playerHeight / 2; }}}}
    ...
    startGamePlay(initData) {
        ...
        // Note: mapMesh check removed, rely on assetsAreReady
        ...
        let iPosX=0, iPosY=0, iPosZ=0; // Position helpers for local player init

        // Get player material (check if exists)
        const playerMaterial = this.world.materials.find(m => m.name === "playerMaterial");
        if (!playerMaterial) {
            console.error("!!! Player physics material not found in world! Interactions may fail.");
            // Optionally create a default here, but ideally it should be defined in initializeCoreComponents
        }

        // Process players from server data
        for(const id in initData.players){
            const sPD = initData.players[id]; // Server Player Data
            // Basic validation of server position data
            if (sPD.x === undefined || sPD.y === undefined || sPD.z === undefined) {
                console.warn(`Player data for ID ${id} is missing position, skipping.`);
                continue;
            }
            ...
            try { // Add try block for body creation
                if(id === localPlayerId){ // --- LOCAL PLAYER ---
                    ...
                    if (!playerBody) throw new Error("Failed to create local player body.");
                    if (playerBody.angularFactor) { playerBody.angularFactor.set(0,1,0); } else { console.warn("playerBody.angularFactor missing"); }
                    ...
                    if(controls?.getObject() && playerBody.position){ // Check playerBody.position
                        controls.getObject().position.copy(playerBody.position); // Copy initial pos
                        controls.getObject().position.y += (CONFIG?.CAMERA_Y_OFFSET ?? 1.6);
                    } else { console.error("Controls object or playerBody position invalid for setting initial view."); }
                    ...
                    if (remotePlayer instanceof ClientPlayer && world) {
                        ...
                        if (!remoteBody) throw new Error(`Failed to create remote player body ${id}.`);

                        const initialRotationY = sPD.rotationY || 0;
                        if (remoteBody.quaternion) { remoteBody.quaternion.setFromEuler(0, initialRotationY, 0); } else { console.warn(`remoteBody.quaternion missing for ${id}`); }

                        this.world.addBody(remoteBody); this.physicsBodies[id] = remoteBody;
                    } else { console.warn(`Skipping physics body for remote player ${id}. Missing ClientPlayer or world.`); }
                } // End if local/remote

            } catch(bodyError) {
                 console.error(`!!! Error creating/configuring physics body for player ${id}:`, bodyError);
                 // Optionally continue to next player or abort entirely?
                 // Aborting might be safer if core physics fails.
                 stateMachine?.transitionTo('homescreen'); UIManager?.showError("Game init failed (physics bodies).", 'homescreen'); return;
            }
...
console.log("game.js loaded (Added Physics Body Checks)");
