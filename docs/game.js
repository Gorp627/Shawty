...
class Game {
    ...
    constructor() {
        ...
        // No need for this.rapierWorld if using global 'rapierWorld' from config.js
        ...
        if (!window.isRapierReady) {
            ...
            if (!window.RAPIER) console.error("RAPIER global missing though flag was ready!");
    ...
    start() {
        ...
        this.mapMesh = null; this.playerRigidBodies = {}; rapierWorld = null; groundCollider = null; this.lastCallTime = performance.now(); // Reset global rapierWorld
    ...
    initializePhysics() {
         ...
         try {
             ...
             rapierWorld = new RAPIER.World(gravity); // Assign to global directly
             if (!rapierWorld) throw new Error("World creation returned null/undefined.");
             rapierEventQueue = new RAPIER.EventQueue(true); if (!rapierEventQueue) throw new Error("EventQueue creation failed."); console.log("[Game] Rapier world & queue created.");

             const groundSize = 100; const groundHeight = 0.1; const groundColliderDesc = RAPIER.ColliderDesc.cuboid(groundSize, groundHeight / 2, groundSize) .setTranslation(0.0, -groundHeight / 2, 0.0) .setFriction(0.5).setRestitution(0.1);
             groundCollider = rapierWorld.createCollider(groundColliderDesc); if (!groundCollider) throw new Error("Ground collider creation failed."); console.log("[Game] Rapier Ground collider created."); return true;
    ...
    attemptProceedToGame() {
        ...
        if (window.RAPIER && rapierWorld && assetsAreReady && networkIsInitialized && initializationData) { // Check ALL flags/instances
        ...
        } else if (assetsAreReady && window.RAPIER && rapierWorld && stateMachine?.is('loading')) { // Assets/Physics done, but waiting on Network/Join
