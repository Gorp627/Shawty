// docs/game.js - Main Game Orchestrator (Log Dimensions, Spawn Higher)
...
// Debug flags
const USE_SIMPLE_GROUND = false; // Use actual map VISUALLY
...
const DEBUG_FORCE_SPAWN_POS = true; // Force spawn
const DEBUG_FORCE_SPAWN_Y = 20.0; // <<< Spawn higher
...
class Game {
    ...
    start() {
        ...
        console.log("[Game] Triggering Asset loading...");
        ...
        this.animate();
        console.log("[Game] Basic setup complete.");
    }
    ...
    bindLoadManagerListeners() {
        ...
        loadManager.on('ready', () => {
            ...
            if (!this.mapMesh && !USE_SIMPLE_GROUND) { console.warn("Map asset data missing (but using simple collider)."); } // Warn but proceed if simple collider

            // Add VISUAL map mesh regardless of collider type
            if (this.mapMesh && this.scene && !this.mapMesh.parent) {
                 this.scene.add(this.mapMesh);
                 console.log("[Game] Added loaded map mesh to scene (visual).");
            }

            this.createMapCollider(); // Create collider (forced simple if flag set)
            this.attemptProceedToGame();
        });
        loadManager.on('error', (d) => { console.error("LoadMgr error:", d); assetsAreReady=false;this.mapMesh=null;if(stateMachine) stateMachine.transitionTo('loading',{message:`Asset Err!`,error:true}); });
    ...
    createMapCollider() {
        ...
        try {
            ...
            let c=rapierWorld.createCollider(colliderDesc); if (!c) throw new Error("Collider create fail."); this.mapColliderHandle=c.handle;
    ...
    startGamePlay(initData) {
        ...
        const ph = CONFIG?.PLAYER_HEIGHT||1.8; const pr = CONFIG?.PLAYER_RADIUS||0.4;
...
console.log("game.js loaded (Log Dimensions, Spawn Higher)");
