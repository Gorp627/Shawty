// docs/game.js - Main Game Orchestrator (Manual Physics Reverted)
...
// var groundCollider = null; // Removed physics global
...
class Game {
    ...
    constructor() {
        ...
        // --- Removed physics instance vars ---
        // this.playerRigidBodies = {};
        // this.rapierWorld = null;
        // this.rapierEventHandlers = null;
    ...
    // --- Removed Physics Initialization ---
    // initializePhysics() { ... }
    ...
    // --- Main Update/Animate Loop ---
     animate() { requestAnimationFrame(() => this.animate()); const dt = this.clock ? this.clock.getDelta() : 0.016;
         // --- REMOVED Physics Step ---
         if(stateMachine?.is('playing')){ try{ if(updateLocalPlayer) updateLocalPlayer(dt, this.mapMesh); /* Pass mapMesh again */ } catch(e){console.error("Err updateLP:",e);} try{ if(updateRemotePlayers) updateRemotePlayers(dt); } catch(e){console.error("Err updateRP:",e);} try{ if(Effects?.update) Effects.update(dt); } catch(e){console.error("Err Effects:",e);}
              // --- REMOVED Visual Sync (handled by updateLocal/Remote) ---
         }
    ...
    startGamePlay(initData) {
        ...
        // Map mesh check IS important again for manual physics
        if (!this.mapMesh) { console.error("!!! MapMesh missing!"); stateMachine?.transitionTo('homescreen'); UIManager?.showError("Init fail (map).", 'homescreen'); return;}

        localPlayerId = initData.id; console.log(`Local ID: ${localPlayerId}`);
        console.log("Clearing previous state...");
        for (const id in players) { if (Network?._removePlayer) Network._removePlayer(id); } players = {}; // Clear players

        let iPosX=0, iPosY=0, iPosZ=0;

        for(const id in initData.players){ const sPD = initData.players[id]; if(id === localPlayerId){ // --- LOCAL ---
                console.log(`Init local: ${sPD.name}`); players[id] = { ...sPD, isLocal: true, mesh: null }; iPosX=sPD.x; iPosY=sPD.y; iPosZ=sPD.z; // Use feet Y from server
                const cameraHeight = CONFIG?.CAMERA_Y_OFFSET || 1.6; const visualY = iPosY + cameraHeight; // Position camera relative to feet
                if(controls?.getObject()){ controls.getObject().position.set(iPosX, visualY, iPosZ); controls.getObject().rotation.set(0, sPD.rotationY || 0, 0); console.log(`Set controls pos(${iPosX.toFixed(1)}, ${visualY.toFixed(1)}, ${iPosZ.toFixed(1)})`); }
                 // <<< RE-ADD Manual Physics State Reset >>>
                velocityY = 0; isOnGround = true; console.log("Reset initial physics state.");
...
console.log("game.js loaded (Reverted to Manual Physics)");
