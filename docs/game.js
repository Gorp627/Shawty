...
class Game {
    ...
    initializeCoreComponents() {
         ...
         try {
             ...
             console.error("!!! Core Init Error:", e); UIManager?.showError(`FATAL: Graphics Init! ${e.message}`, 'loading'); return false;
         }
    }

    // --- Initialize Network ---
    initializeNetwork() {
        console.log("Init Network..."); if (Network?.init) { try { Network.init(); console.log("Net init ok."); return true; } catch (e) { console.error("Net Init Err:", e); stateMachine?.transitionTo('loading', { message: `Net Fail! ${e.message}`, error: true }); return false; } }
        else { console.error("Network missing!"); stateMachine?.transitionTo('loading', { message: `Net Load Fail!`, error: true }); return false; }
    }
    ...
    bindLoadManagerListeners() {
        ...
        loadManager.on('ready', () => { console.log("LoadMgr 'ready'."); assetsAreReady = true; this.mapMesh = loadManager.getAssetData('map'); if (!this.mapMesh) { console.error("Map data missing!"); stateMachine?.transitionTo('loading', { message: "Map Data Fail!", error: true }); return; } console.log("Map mesh stored."); this.attemptProceedToGame(); });
     ...
     animate() {
        requestAnimationFrame(() => this.animate()); const dt = this.clock ? this.clock.getDelta() : 0.016;
         if(stateMachine?.is('playing')){
             try{ if(updateLocalPlayer) updateLocalPlayer(dt, this.mapMesh); /* Pass mapMesh */ } catch(e){console.error("Err updateLP:",e);}
    ...
    startGamePlay(initData) {
        ...
        if (!this.mapMesh) { console.error("MapMesh missing for spawn!"); stateMachine?.transitionTo('homescreen'); UIManager?.showError("Init fail (map).", 'homescreen'); return;} // Crucial check
        ...
        // --- Spawn Logic ---
        let spawnX = 0, spawnY = 0, spawnZ = 0; // Default spawn point

        // Find local player's data to get target XZ
        const localPlayerData = initData.players[localPlayerId];
        if (localPlayerData) {
            spawnX = localPlayerData.x;
            spawnY = localPlayerData.y; // Use server Y as fallback
            spawnZ = localPlayerData.z;

            // --- Calculate Spawn Height via Raycast ---
            const spawnCheckHeight = 100.0;
            ...
            raycaster.far = spawnCheckHeight + 50;

            console.log("Casting spawn ray at", spawnX, spawnZ);
            const intersects = raycaster.intersectObject(this.mapMesh, true);

            if (intersects.length > 0) {
                spawnY = intersects[0].point.y; // Found ground! Use its Y coordinate
                console.log(`Spawn ray hit! Ground Y: ${spawnY.toFixed(2)}`);
            } else {
                console.warn(`Spawn ray MISSED at X:${spawnX}, Z:${spawnZ}. Using default Y=${spawnY}`);
                spawnY = 0; // Fallback to Y=0 if ray misses
            }
            // --- End Spawn Height Calculation ---
        } else {
            console.error("Local player data missing in initialization data!");
            // Handle error - perhaps transition back to homescreen?
            stateMachine?.transitionTo('homescreen'); UIManager?.showError("Init fail (local data).", 'homescreen'); return;
        }


        // Process all players (position local player using calculated spawnY)
        for(const id in initData.players){
            const sPD = initData.players[id];
            if(id === localPlayerId){
                console.log(`Init local: ${sPD.name}`);
                players[id] = { ...sPD, isLocal: true, mesh: null }; // Store data
                players[id].y = spawnY; // <<< OVERWRITE server Y with calculated ground Y
                ...
                if(controls?.getObject()){
                    controls.getObject().position.set(spawnX, visualY, spawnZ); // Use calculated Y
                    controls.getObject().rotation.set(0, sPD.rotationY || 0, 0);
                    console.log(`Set controls pos(${spawnX.toFixed(1)}, ${visualY.toFixed(1)}, ${spawnZ.toFixed(1)})`);
                }
            ...
            } else { // Remote players - Use server Y for their setup
...
console.log("game.js loaded (Reverted to Manual Physics + Spawn Raycast)");
