// docs/game.js - Main Game Orchestrator (with Rapier.js - Corrected Rotation API)
...
class Game {
    ...
    constructor() {
        ...
        if (!this.rapierReady) {
            window.addEventListener('rapier-ready', () => {
                ...
                if (!RAPIER) console.error("RAPIER missing after event!");
                else { this.initializePhysics(); this.attemptProceedToGame(); }
            }, { once: true });
        } else {
            if (!window.RAPIER) console.error("RAPIER ready but global missing!");
            else this.initializePhysics(); // Initialize physics right away if RAPIER already loaded
            console.log("[Game] Rapier ready on construct.");
        }
    }
    ...
    start() {
        ...
        this.mapMesh = null; this.playerRigidBodyHandles = {}; this.mapColliderHandle = null;
        rapierWorld = null; groundColliderHandle = null; rapierEventQueue = null; // Reset globals
        this.lastCallTime = performance.now();
        ...
        console.log("Triggering Asset and Rapier loading...");
        this.startAssetLoading(); // Assets load in parallel
        if(stateMachine) stateMachine.transitionTo('loading', {message:"Loading Engine..."}); else console.error("stateMachine missing!");
        this.animate(); // Start render loop (physics waits)
        console.log("Basic setup done, waiting for loads...");
    }
    ...
    bindLoadManagerListeners() {
        ...
        loadManager.on('ready', () => {
            ...
            this.mapMesh = loadManager.getAssetData('map'); if (!this.mapMesh) { console.error("Map data missing!"); stateMachine?.transitionTo('loading', { message: "Map Data Fail!", error: true }); return; } console.log("Visual map mesh stored.");
            ...
            if (RAPIER && rapierWorld && this.mapMesh) {
                try {
                    ...
                    this.mapMesh.traverse((child) => { if (!foundMesh && child.isMesh && child.geometry) { let geometry = child.geometry; let vertices = geometry.attributes.position.array; let indices = geometry.index ? geometry.index.array : null; console.log(`Found map geometry: ${child.name || '(Unnamed)'}`); if (vertices && indices) { let desc = RAPIER.ColliderDesc.trimesh(vertices, indices).setFriction(0.7).setRestitution(0.1); this.mapColliderHandle = rapierWorld.createCollider(desc)?.handle; } else if (vertices) { console.warn("Map geometry has no indices."); let desc = RAPIER.ColliderDesc.trimesh(vertices).setFriction(0.7).setRestitution(0.1); this.mapColliderHandle = rapierWorld.createCollider(desc)?.handle; } else { throw new Error("Mesh has no vertices!"); } if (this.mapColliderHandle !== undefined) console.log(`Created map collider handle: ${this.mapColliderHandle}`); else throw new Error("Collider creation failed."); foundMesh = true; } }); if (!foundMesh) console.warn("No suitable mesh found in map GLB for collider.");
                } catch(e) { console.error("!!! Error creating map collider:", e); stateMachine?.transitionTo('loading', { message: "Map Physics Fail!", error: true }); return; }
            } else { console.error("Cannot create map collider: RAPIER/World/MapMesh not ready!"); }

            this.attemptProceedToGame(); // Check if ready to proceed now
        });
        loadManager.on('error', (data) => { console.error("LoadMgr error:", data); assetsAreReady = false; this.mapMesh = null; this.mapColliderHandle = null; stateMachine?.transitionTo('loading',{message:`Asset Err!<br/>${data.message||''}`,error:true}); }); console.log("LoadMgr listeners bound.");
    }
...
console.log("game.js loaded (Rapier - Euler Rotation Fix)");
