// docs/game.js - Main Game Orchestrator (with Rapier.js - Re-add Map to Scene)
...
class Game {
    ...
    start() {
        ...
        this.startAssetLoading(); // Assets load in parallel with Rapier init
    ...
    bindLoadManagerListeners() {
        ...
        loadManager.on('ready', () => {
            ...
            // ---<<< ADD MAP MESH TO SCENE HERE >>>---
            if (this.scene && this.mapMesh) {
                console.log("[Game Ready] Adding visual mapMesh to scene.");
                this.scene.add(this.mapMesh); // Add the loaded map object to the Three.js scene
                console.log("[Game Ready] Visual mapMesh added.");
                // Optional: Log children count or specific object check
                console.log("[Game Ready] Scene children count:", this.scene.children.length);
            } else {
                console.error("[Game Ready] Cannot add mapMesh! Scene or mapMesh missing!");
            }
            // ---<<< END ADD TO SCENE >>>---
            ...
            if (RAPIER && rapierWorld && this.mapMesh) {
                try {
                    ...
                    this.mapMesh.traverse((child) => { if (!foundMesh && child.isMesh && child.geometry) { let g=child.geometry; let v=g.attributes.position.array; let i=g.index?g.index.array:null; console.log(`Found map geom: ${c.name||'?'}`); let desc; if(v&&i){desc=RAPIER.ColliderDesc.trimesh(v,i);}else if(v){console.warn("Map geom no indices.");desc=RAPIER.ColliderDesc.trimesh(v);}else{throw new Error("Mesh no vertices!");} desc.setFriction(0.7).setRestitution(0.1); this.mapColliderHandle=rapierWorld.createCollider(desc)?.handle; if(this.mapColliderHandle===undefined)throw new Error("Collider create fail."); console.log(`Map collider handle: ${this.mapColliderHandle}`); foundMesh=true;}}); if (!foundMesh) console.warn("No suitable mesh found for map collider.");
...
console.log("game.js loaded (Re-added Map Add)");
