...
class Game {
    ...
    start() {
        ...
        // <<< RE-ADD CALL TO initializeCoreComponents >>>
    ...
    // --- Initialize Core Components (Three.js ONLY) <<< RE-ADDED METHOD >>> ---
    initializeCoreComponents() {
         ...
         try {
             ...
             console.log("[Game] Three.js Core Components OK.");
    ...
    startGamePlay(initData) {
        ...
        for(const id in initData.players){ const sPD = initData.players[id]; if(id === localPlayerId){
                console.log(`Init local: ${sPD.name}`); players[id] = { ...sPD, isLocal: true, mesh: null }; iPosX=sPD.x; iPosY=sPD.y; iPosZ=sPD.z;
                const cameraHeight = CONFIG?.CAMERA_Y_OFFSET || 1.6; const visualY = iPosY + cameraHeight;
                ...
                velocityY = 0; isOnGround = true; console.log("Reset initial physics state."); // Reset manual physics state
