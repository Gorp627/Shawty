...
class Game {
    ...
    // --- Initialize Network ---
    initializeNetwork() {
        console.log("Init Network..."); if (typeof Network !== 'undefined' && typeof Network.init === 'function') { try { Network.init(); console.log("Net init ok."); return true; } catch (e) { console.error("Net Init Err:", e); if(typeof stateMachine !== 'undefined') stateMachine.transitionTo('loading', { message: `Net Fail! ${e.message}`, error: true }); return false; } } else { console.error("Network missing!"); if(typeof stateMachine !== 'undefined') stateMachine.transitionTo('loading', { message: `Net Load Fail!`, error: true }); return false; }
    }

    // --- Setup Asset Loading ---
    bindLoadManagerListeners() {
        console.log("[Game] Binding LoadManager listeners...");
        if (typeof loadManager === 'undefined') { console.error("LoadMgr missing!"); if(typeof stateMachine !== 'undefined') stateMachine.transitionTo('loading',{message:"Load Mgr Fail!", error:true}); return; }
        loadManager.on('ready', () => {
            ...
            if (!this.mapMesh) { console.error("Map data missing after ready!"); if(typeof stateMachine !== 'undefined') stateMachine.transitionTo('loading', { message: "Map Data Fail!", error: true }); return; }
            ...
            if (this.scene && this.mapMesh) {
                ...
                let foundInScene = this.scene.children.includes(this.mapMesh); console.log("[Game Ready] Is mapMesh in scene.children now?", foundInScene); if(!foundInScene) console.error("!!! Map mesh FAILED to add to scene!");
                console.log(`[Game Ready] Map Mesh Pos: ${this.mapMesh.position.x.toFixed(1)},${this.mapMesh.position.y.toFixed(1)},${this.mapMesh.position.z.toFixed(1)}`); console.log(`[Game Ready] Map Mesh Scale: ${this.mapMesh.scale.x.toFixed(1)},${this.mapMesh.scale.y.toFixed(1)},${this.mapMesh.scale.z.toFixed(1)}`);
            } else { console.error("[Game Ready] Cannot add mapMesh! Scene:", this.scene, "MapMesh:", this.mapMesh); }

            this.attemptProceedToGame(); // Check if ready to start/go home
        });
        loadManager.on('error', (data) => {
             console.error("LoadMgr error:", data); assetsAreReady = false; this.mapMesh = null; if(typeof stateMachine !== 'undefined') stateMachine.transitionTo('loading',{message:`Asset Err!<br/>${data.message||''}`,error:true});
        });
    ...
    attemptProceedToGame() {
        console.log(`Check Proceed: Assets=${assetsAreReady}, NetInit=${networkIsInitialized}, Data=${!!initializationData}`); if (assetsAreReady && networkIsInitialized && initializationData) { console.log("All Ready -> startGamePlay"); if (currentGameInstance?.startGamePlay) { currentGameInstance.startGamePlay(initializationData); } else { console.error("Game instance missing!"); } } else if (assetsAreReady && typeof stateMachine !== 'undefined' && stateMachine.is('loading')) { console.log("Assets Ready -> Homescreen"); let pC = '?'; if (typeof UIManager !== 'undefined' && UIManager?.playerCountSpan) pC=UIManager.playerCountSpan.textContent??'?'; stateMachine.transitionTo('homescreen', { playerCount: pC }); } else { console.log(`Not ready state: ${typeof stateMachine !== 'undefined' ? stateMachine.currentState : 'Unknown'}`); }
    }

    // --- Initialize Managers ---
    initializeManagers() {
        console.log("Init Mgrs..."); if(!UIManager||!Input||!stateMachine||!loadManager||!Network||!Effects) { console.error("Mgr undef!"); if(typeof UIManager !== 'undefined' && UIManager?.showError) UIManager.showError("FATAL: Mgr Load!", 'loading'); return false; } try { if(!UIManager.initialize()) throw new Error("UI fail"); Input.init(this.controls); Effects.initialize(this.scene); console.log("Mgr Initialized."); return true; } catch (e) { console.error("Mgr Init Err:", e); if(typeof UIManager !== 'undefined' && UIManager?.showError) UIManager.showError(`FATAL: Mgr Setup! ${e.message}`, 'loading'); return false; }
    }
        ...
        if (typeof stateMachine !== 'undefined') {
            stateMachine.on('transition', (data) => {
                 ...
                 if(data.to==='homescreen'){
                     ...
                     if(data.from==='playing'||data.from==='joining'){ console.log(`Cleanup after ${data.from}...`); for(const id in players){if(id!==localPlayerId&&Network?._removePlayer){Network._removePlayer(id);}} if(players?.[localPlayerId]){delete players[localPlayerId];} players={}; localPlayerId=null; if(controls?.isLocked) controls.unlock(); console.log("State cleared.");}
                 } else if(data.to==='playing'){
                     if(UIManager&&localPlayerId&&players?.[localPlayerId]){UIManager.updateHealthBar(players[localPlayerId].health); UIManager.updateInfo(`Playing as ${players[localPlayerId].name}`);}
                 } else if(data.to==='loading'&&data.options?.error){ console.error("Loading error:", data.options.message); if(controls?.isLocked)controls.unlock(); networkIsInitialized=false; assetsAreReady=false; initializationData=null; this.mapMesh=null; players={}; localPlayerId=null; }
            });
    ...
    // --- Add Event Listeners ---
    addEventListeners() {
        console.log("Add listeners..."); if(typeof UIManager !== 'undefined' && UIManager?.joinButton && typeof Network !== 'undefined' && Network?.attemptJoinGame){ UIManager.joinButton.addEventListener('click', ()=>{ if (!assetsAreReady) { UIManager.showError("Assets loading...", 'homescreen'); return; } Network.attemptJoinGame(); }); console.log("Join listener.");} else { console.error("Cannot add join listener!"); }
...
console.log("game.js loaded (Manual Physics - Explicit Globals)");
