...
class Game {
    ...
    bindLoadManagerListeners() {
        ...
        loadManager.on('ready', () => {
            ...
            if (networkIsInitialized && initializationData) {
                console.log("[Game LoadReady] Case 1: Net init done. Starting game...");
                if (currentGameInstance?.startGamePlay) { currentGameInstance.startGamePlay(initializationData); } else { console.error("Game instance missing!"); }
                 ...
                 console.log("[Game LoadReady] Case 2: Joining state & connected. Sending details...");
                 Network.sendJoinDetails();
            } else if (stateMachine?.is('loading')) { // Check specifically for 'loading'
                // Default: Assets ready, still in loading screen -> Go to Homescreen
                console.log("[Game LoadReady] Case 3: Still loading state. Attempting transition to HomeScreen...");
                // Simplify the call first to ensure it's not the args causing issues
                try {
                    stateMachine.transitionTo('homescreen'); // Try without extra args first
                    console.log("[Game LoadReady] Transition to homescreen CALLED.");
                    // If the above works, re-add args carefully:
                    // stateMachine.transitionTo('homescreen', { playerCount: UIManager?.playerCountSpan?.textContent ?? '?' });
                } catch (e) {
                     console.error("!!! Error during transitionTo homescreen call:", e);
                }
            } else {
                 console.log(`[Game LoadReady] Case 4: State is '${stateMachine?.currentState}'. No immediate action needed.`);
            }
        });

        loadManager.on('error', (data) => {
            console.error("[Game] LoadManager 'error' event received.", data); assetsAreReady = false; this.mapMesh = null;
            stateMachine?.transitionTo('loading',{message:`FATAL: Asset Error!<br/>${data.message||'Check console.'}`,error:true});
        });
    }

    // --- Centralized logic (OBSOLETE/REMOVED) ---
    ...
    initializeCoreComponents() {
         ...
         try { // Three.js setup...
             ...
             // Cannon.js Setup...
             if (typeof CANNON === 'undefined') throw new Error("Cannon lib not loaded!"); this.world = new CANNON.World(); world = this.world; this.world.gravity.set(0, CONFIG?.GRAVITY ?? -9.82, 0); this.world.broadphase = new CANNON.NaiveBroadphase(); const groundMaterial = new CANNON.Material("groundMaterial"); const playerMaterial = new CANNON.Material("playerMaterial"); const groundPlayerContactMaterial = new CANNON.ContactMaterial(groundMaterial, playerMaterial, { friction: 0.1, restitution: 0.1 }); this.world.addContactMaterial(groundPlayerContactMaterial); const groundShape = new CANNON.Plane(); const groundBody = new CANNON.Body({ mass: 0, shape: groundShape, material: groundMaterial }); groundBody.quaternion.setFromAxisAngle(new CANNON.Vec3(1, 0, 0), -Math.PI / 2); groundBody.position.set(0, 0, 0); this.world.addBody(groundBody); console.log("[Game] Cannon.js World OK."); return true;
    ...
    bindOtherStateTransitions() {
        ...
        if (stateMachine) {
            stateMachine.on('transition', (data) => {
                 console.log(`[Game State Listener] Transition EVENT: ${data.from} -> ${data.to}`); // Log the actual event firing
                 if (data.to === 'homescreen') {
                    ...
                    console.error("Loading error state:", data.options.message); if(controls?.isLocked)controls.unlock(); networkIsInitialized = false; assetsAreReady = false; initializationData = null; this.mapMesh = null; this.physicsBodies = {};
                }
            });
    ...
    startGamePlay(initData) {
        ...
        // Note: Removed mapMesh check - relying on assetsAreReady having been true
        ...
        for(const id in initData.players){
            ...
            try { // --- Start Body Creation Try ---
                if(id === localPlayerId){ // Local
                    console.log(`[Game] Init local player: ${sPD.name}`); players[id] = { ...sPD, isLocal: true, mesh: null }; iPosX=sPD.x; iPosY=sPD.y; iPosZ=sPD.z;
                    const playerShape = new CANNON.Sphere(playerRadius); const playerBody = new CANNON.Body({ mass: CONFIG?.PLAYER_MASS||70, position: new CANNON.Vec3(sPD.x, bodyCenterY, sPD.z), shape: playerShape, material: playerMaterial, linearDamping: 0.5, angularDamping: 0.9 });
...
console.log("game.js loaded (Revised Ready Handler Logic 2)");
