// docs/game.js - Main Game Orchestrator (Log/Validate Physics Pos - FULL FILE v4)
...
class Game {
    ...
    syncCameraToBody(playerBody) {
     ...
     cleanupGameState() {
         ...
         // ** Don't reset mapColliderHandle here, it persists unless map reload **
         console.log("[Game Cleanup] Player state cleared.");
     }
...
function runGame() { console.log("--- runGame() invoked ---"); try { if (window.currentGameInstance) { console.warn("!!! Previous game instance found."); } const gameInstance = new Game(); window.currentGameInstance = gameInstance; gameInstance.start(); } catch (e) { console.error("!!! CRITICAL Error Creating Game Instance:", e); document.body.innerHTML = `<p style='color:red; font-size: 1.5em; text-align: center; padding: 20px;'>FATAL ERROR: GAME INITIALIZATION FAILED.<br/>Check console (F12) for details.</p>`; } }
// --- DOM Ready Check ---
if (document.readyState === 'loading') { document.addEventListener('DOMContentLoaded', runGame); } else { runGame(); }

console.log("game.js loaded (Log/Validate Physics Pos - FULL FILE v4)");
