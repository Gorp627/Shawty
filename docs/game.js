...
var RAPIER = window.RAPIER || null; // Initialize from window object set by rapier_init.js
// Global rapierWorld is declared in config.js and assigned by Game instance's initializePhysics
var rapierWorld = null;
var rapierEventQueue = null; // Event queue for collisions
...
class Game {
    ...
    constructor() {
        ...
        if (!this.rapierReady) {
            window.addEventListener('rapier-ready', () => {
                ...
                RAPIER = window.RAPIER; // Ensure global ref is updated from window
                if (!RAPIER) {
                    console.error("FATAL: RAPIER global missing after ready event!");
                    stateMachine?.transitionTo('loading', { message: "FATAL: Physics Load Error!", error: true });
                } else {
             ...
             // If already ready when Game constructed, ensure global is assigned and try initializing physics
    ...
    start() {
        ...
        this.mapMesh = null; this.playerRigidBodies = {}; rapierWorld = null; groundCollider = null; rapierEventQueue = null; this.lastCallTime = performance.now(); // Reset globals too
        ...
        // Start animation loop - it will wait for Rapier World creation before stepping physics
        this.animate();
    ...
    initializePhysics() {
         if (!RAPIER) { console.error("Attempted physics init but RAPIER global not ready!"); return false; } // Use global check
         if (rapierWorld) { console.warn("Physics world already initialized."); return true; } // Check global world

         console.log("[Game] Initializing Rapier Physics World...");
         try {
             //const RAPIER = window.RAPIER; // Already checked global RAPIER exists
             const gravity = new RAPIER.Vector3(0.0, CONFIG?.GRAVITY ?? -9.81, 0.0);
             rapierWorld = new RAPIER.World(gravity); // Assign to global var
             ...
             const groundColliderDesc = RAPIER.ColliderDesc.cuboid(groundSize, groundHeight / 2, groundSize)
                 ...
                 .setFriction(0.7) // Slightly more friction for ground
                 .setRestitution(0.1);
             groundCollider = rapierWorld.createCollider(groundColliderDesc); // Create collider, store handle
             ...
             rapierWorld = null; rapierEventQueue = null; groundCollider = null; // Nullify globals on error
    ...
    attemptProceedToGame() {
        console.log(`[Game] Check Proceed: Rapier=${!!RAPIER}, World=${!!rapierWorld}, Assets=${assetsAreReady}, NetInit=${networkIsInitialized}, Data=${!!initializationData}`);
        if (RAPIER && rapierWorld && assetsAreReady && networkIsInitialized && initializationData) {
        ...
        } else if (assetsAreReady && RAPIER && rapierWorld && stateMachine?.is('loading')) { // If done loading core parts but waiting for join/init
     ...
     animate() {
        ...
        if(rapierWorld && stateMachine?.is('playing')){
            try{ // Update Local Player
                ...
                // Get the actual RigidBody object using the handle
                const localPlayerBody = localPlayerHandle !== undefined ? rapierWorld.getRigidBody(localPlayerHandle) : null;
            ...
            try{ // Update Effects
             ...
             const localBody = localBodyHandle !== undefined ? rapierWorld.getRigidBody(localBodyHandle) : null;
             // Sync Local Controls
             ...
             // Sync Remote Meshes
             for (const id in players) { if (id !== localPlayerId && players[id] instanceof ClientPlayer && players[id].mesh) { const rbHandle = this.playerRigidBodies[id]; const rb = rbHandle !== undefined ? rapierWorld.getRigidBody(rbHandle) : null; if (rb) { const p=rb.translation(); const r=rb.rotation(); players[id].mesh.position.set(p.x,p.y,p.z); players[id].mesh.quaternion.set(r.x, r.y, r.z, r.w); const h=CONFIG?.PLAYER_HEIGHT||1.8; if(!(players[id].mesh.geometry instanceof THREE.CylinderGeometry)) {players[id].mesh.position.y -= h/2.0;} }}}
        }
    ...
    startGamePlay(initData) {
        ...
        // Use correct method to remove bodies by handle
        ...
        for (const id in players) { if (Network?._removePlayer) Network._removePlayer(id); } players = {}; // Clear players AFTER removing visuals/bodies
                ...
                if(id === localPlayerId){ // --- LOCAL ---
                    ...
                    if(UIManager){ UIManager.updateHealthBar(sPD.health ?? 100); UIManager.updateInfo(`Playing as ${sPD.name}`); UIManager.clearError('homescreen'); UIManager.clearKillMessage(); }
                     ...
                     if (remotePlayer instanceof ClientPlayer && rapierWorld) {
                         ...
                         const q = RAPIER.Quaternion.fromEulerAngles(0, rotY, 0); // USE fromEulerAngles for object input
                         let rigidBodyDesc = RAPIER.RigidBodyDesc.kinematicPositionBased().setTranslation(sPD.x, bodyCenterY, sPD.z).setRotation(q); // Pass quat
