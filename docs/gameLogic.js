...
function updateLocalPlayer(deltaTime) {
    ...
    const rayOrigins = [
        ...
        new THREE.Vector3(currentPos.x + playerRadius * 0.7, playerBaseY + aboveFeetOffset, currentPos.z), // Right
    ...
    if (isMapReady) {
        ...
        for (let i = 0; i < rayOrigins.length; i++) {
             ...
             try {
                 ...
                 if (intersects.length > 0) {
                     ...
                     if (hitFoundForThisRay && closestHitDist < groundCheckDistance) { // Hit valid ground below origin
                         onValidGround = true; // Set flag if ANY ray hits valid ground
                         highestGroundY = Math.max(highestGroundY, hitPointY);
                         rayHits[i] = true; // Mark this ray as hit for debug
                     }
                 }
        ...
        if (onValidGround) {
            if (playerBaseY <= highestGroundY + groundSnapThreshold) { // If feet are below highest detected point + threshold
    ...
    const currentRotation = new THREE.Euler().setFromQuaternion(controlsObject.quaternion,'YXZ'); const currentRotationY = currentRotation.y; const lastSentState = playerState;
    const pTSq=CONFIG.PLAYER_MOVE_THRESHOLD_SQ||0.0001; const rTh=0.01;
    const pChg=logicalPosition.distanceToSquared(new THREE.Vector3(lastSentState?.x??0,lS?.y??0,lS?.z??0)) > pTSq; const rChg=Math.abs(currentRotationY-(lastSentState?.rotationY??0)) > rTh;
    if(pChg||rChg){ if(lastSentState){lastSentState.x=logicalPosition.x;lastSentState.y=logicalPosition.y;lastSentState.z=logicalPosition.z;lastSentState.rotationY=currentRotationY;} if(Network)Network.sendPlayerUpdate({x:logicalPosition.x,y:logicalPosition.y,z:logicalPosition.z,rotationY:currentRotationY}); }
