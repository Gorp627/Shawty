...
function loadPlayerModel() {
    ...
    loader.load(PLAYER_MODEL_PATH,(gltf)=>{ console.log("Player model OK!");playerModel=gltf.scene;playerModel.traverse((c)=>{if(c.isMesh)c.castShadow=true;});playerModelLoadState='loaded';checkAssetsReady(); },undefined,(err)=>{ console.error("!!! Player model LOAD ERROR:",err);playerModelLoadState='error';checkAssetsReady(); });
}
...
function loadGunModel() {
    ...
    loader.load(GUN_MODEL_PATH,(gltf)=>{ console.log("Gun model OK!");gunModel=gltf.scene;gunModel.traverse((c)=>{if(c.isMesh){c.castShadow=false;c.receiveShadow=false;}});gunModelLoadState='loaded';checkAssetsReady(); },undefined,(err)=>{ console.error("!!! Gun model LOAD ERROR:",err);gunModelLoadState='error';checkAssetsReady(); });
}

function loadMap(mapPath) {
    mapLoadState = 'loading'; console.log(`Loading map: ${mapPath}`);
    if (!loader) { console.error("! Loader not init before loadMap"); return; }
    loader.load(mapPath,(gltf)=>{ console.log("Map OK!");mapMesh=gltf.scene;mapMesh.traverse((c)=>{if(c.isMesh){c.castShadow=true;c.receiveShadow=true;c.userData.isCollidable=true;}});scene.add(mapMesh);mapLoadState='loaded';checkAssetsReady(); },undefined,(err)=>{ console.error(`!!! Map LOAD ERROR (${mapPath}):`,err);mapLoadState='error';checkAssetsReady(); });
}
...
function checkAssetsReady() {
    ...
    if (mapR && pModelR && gModelR) { // Check all assets
...
function setupSocketIO() {
    ...
    socket.on('connect', () => {
        ...
        checkAssetsReady(); // Re-check if assets are ready now
    });
    ...
    socket.on('playerCountUpdate', (count) => {
        ...
        playerCountSpan = playerCountSpan || document.getElementById('playerCount');
        if (playerCountSpan) playerCountSpan.textContent = count; else console.warn("playerCountSpan missing");
    ...
    socket.on('initialize', (data) => {
        console.log('Initialize received.'); localPlayerId = data.id;
        ...
        for (const id in data.players) {
            ...
            if (id === localPlayerId) {
                ...
                iPosX = pD.x; iPosY = pD.y; iPosZ = pD.z;
                const visY = iPosY + PLAYER_HEIGHT;
                if (controls?.getObject()) { controls.getObject().position.set(iPosX, visY, iPosZ); console.log(`Set controls pos: ${iPosX},${visY},${iPosZ}`); } else console.error("Controls missing for init pos!");
                velocityY = 0; isOnGround = true; updateHealthBar(pD.health); infoDiv.textContent = `Playing as ${localPlayerName}`;
            } else { addPlayer(pD); }
        }
        console.log("Game initialized with players:", Object.keys(players).length);
        setGameState('playing');
    });

    socket.on('playerJoined', (d) => { handlePlayerJoined(d); });
    socket.on('playerLeft', (id) => { handlePlayerLeft(id); });
    socket.on('playerMoved', (d) => { updateRemotePlayerPosition(d); });
    socket.on('shotFired', (d) => { spawnBullet(d); });
    socket.on('healthUpdate', (d) => { handleHealthUpdate(d); });
    socket.on('playerDied', (d) => { handlePlayerDied(d); });
    socket.on('playerRespawned', (d) => { handlePlayerRespawned(d); });
} // End setupSocketIO
...
function shoot() {
    ...
    if(gunViewModel){gunViewModel.getWorldPosition(bulPos);const muzzleOffset=new THREE.Vector3(0,-0.05,-0.5).applyQuaternion(camera.quaternion);bulPos.add(muzzleOffset);console.log("Bullet origin: gun");}else{camera.getWorldPosition(bulPos);console.log("Bullet origin: cam");} camera.getWorldDirection(bulDir);
...
function handleHealthUpdate(data) { if(players[data.id]){players[data.id].health=data.health;if(data.id===localPlayerId){updateHealthBar(data.health);console.log(`Local health UI updated: ${data.health}`);}/* else console.log(`P ${data.id} health -> ${data.health}`);*/}}
...
function init() {
    ...
    setGameState('loading'); // Start in loading state
    ...
    try {
        ...
        loader = new THREE.GLTFLoader(); // Initialize GLTFLoader
        dracoLoader = new THREE.DRACOLoader(); // Initialize DRACOLoader
        dracoLoader.setDecoderPath('https://cdn.jsdelivr.net/npm/three@0.128.0/examples/js/libs/draco/');
        dracoLoader.setDecoderConfig({ type: 'js' });
        loader.setDRACOLoader(dracoLoader); // Associate Draco with GLTF loader
        console.log("Three.js core initialized (incl loaders).");
    } catch (e) { console.error("CRITICAL ERROR during Three.js init:", e); setGameState('loading',{message: "FATAL: Graphics Init Error!", error: true}); return; }
