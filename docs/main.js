...
function setGameState(newState, options = {}) {
    ...
    gameState = newState;
    playerCountSpan = playerCountSpan || document.getElementById('playerCount'); joinButton = joinButton || document.getElementById('joinButton'); // Define these earlier
    ...
    switch(newState){
        ...
        case'homescreen':if(homeScreen){homeScreen.style.display='flex';homeScreen.classList.add('visible');if(playerCountSpan)playerCountSpan.textContent=options.playerCount??playerCountSpan.textContent??'?';if(controls?.isLocked)controls.unlock();const obj=scene?.getObjectByName("PlayerControls");if(obj)scene.remove(obj);removeGunViewModel();if(joinButton){joinButton.disabled=false;joinButton.textContent="Join Game";}}break;
...
function loadPlayerModel() { playerModelLoadState='loading'; if(!loader)return; loader.load(PLAYER_MODEL_PATH, function(gltf){ playerModel=gltf.scene; playerModel.traverse(function(c){if(c.isMesh)c.castShadow=true;}); playerModelLoadState='loaded'; checkAssetsReady(); }, undefined, function(err){ console.error("P Model ERR:",err); playerModelLoadState='error'; checkAssetsReady(); });}
function loadGunModel() { gunModelLoadState='loading'; if(!loader)return; loader.load(GUN_MODEL_PATH, function(gltf){ gunModel=gltf.scene; gunModel.traverse(function(c){if(c.isMesh){c.castShadow=false;c.receiveShadow=false;}}); gunModelLoadState='loaded'; checkAssetsReady(); }, undefined, function(err){ console.error("G Model ERR:",err); gunModelLoadState='error'; checkAssetsReady(); });}
function loadMap(mapPath) { mapLoadState='loading'; if(!loader)return; loader.load(mapPath, function(gltf){ mapMesh=gltf.scene; mapMesh.traverse(function(c){if(c.isMesh){c.castShadow=true;c.receiveShadow=true;c.userData.isCollidable=true;}}); scene.add(mapMesh); mapLoadState='loaded'; checkAssetsReady(); }, undefined, function(err){ console.error(`Map ERR (${mapPath}):`,err); mapLoadState='error'; checkAssetsReady(); });}
function checkAssetsReady() { const mR=mapLoadState==='loaded'||mapLoadState==='error';const pR=playerModelLoadState==='loaded'||playerModelLoadState==='error';const gR=gunModelLoadState==='loaded'||gunModelLoadState==='error'; if(mR&&pR&&gR){ if(mapLoadState==='error'||playerModelLoadState==='error'||gunModelLoadState==='error'){ assetsReady=false; console.error("Asset fail."); setGameState('loading',{message:"FATAL: Asset Error!",error:true}); } else { assetsReady=true; console.log("Assets OK."); if(socket?.connected && gameState==='loading'){ setGameState('homescreen',{playerCount:playerCountSpan?.textContent??'?'});} else if(gameState==='joining'){ sendJoinDetails();}}} else { assetsReady=false; } }

// --- Network & Joining ---
function setupSocketIO() { socket=io(SERVER_URL,{transports:['websocket'],autoConnect:true}); socket.on('connect',function(){console.log('Socket connected:',socket.id);checkAssetsReady();}); socket.on('disconnect',function(r){console.warn('Disconnected:',r);setGameState('homescreen',{playerCount:0});infoDiv.textContent='Disconnected';for(const id in players)removePlayerMesh(id);players={};bullets=[];}); socket.on('connect_error',function(e){console.error('Connect Err:',e.message);mapLoadState='error';playerModelLoadState='error';gunModelLoadState='error';assetsReady=false;setGameState('loading',{message:`Connect Fail!<br/>${e.message}`,error:true});}); socket.on('playerCountUpdate',function(c){playerCountSpan=playerCountSpan||document.getElementById('playerCount');if(playerCountSpan)playerCountSpan.textContent=c;if(assetsReady&&socket.connected&&gameState==='loading')setGameState('homescreen',{playerCount:c});}); socket.on('initialize',function(d){handleInitialize(d);}); socket.on('playerJoined',function(d){handlePlayerJoined(d);}); socket.on('playerLeft',function(id){handlePlayerLeft(id);}); socket.on('playerMoved',function(d){updateRemotePlayerPosition(d);}); socket.on('shotFired',function(d){spawnBullet(d);}); socket.on('healthUpdate',function(d){handleHealthUpdate(d);}); socket.on('playerDied',function(d){handlePlayerDied(d);}); socket.on('playerRespawned',function(d){handlePlayerRespawned(d);}); }
function handleInitialize(data) { localPlayerId=data.id; for(const id in players)removePlayerMesh(id); players={};bullets=[]; let x=0,y=0,z=0; for(const id in data.players){ const pD=data.players[id]; if(id===localPlayerId){ players[id]={...pD,name:localPlayerName,phrase:localPlayerPhrase,mesh:null}; x=pD.x;y=pD.y;z=pD.z; const vY=y+PLAYER_HEIGHT; if(controls?.getObject())controls.getObject().position.set(x,vY,z);else console.error("Controls missing!"); velocityY=0;isOnGround=true; updateHealthBar(pD.health); infoDiv.textContent=`Playing as ${localPlayerName}`; }else{addPlayer(pD);}} console.log("Game init players:",Object.keys(players).length); setGameState('playing');}
...
// --- Player Management ---
function addPlayer(pD) { if(players[pD.id]||pD.id===localPlayerId)return; players[pD.id]={...pD,mesh:null,targetPosition:null,targetRotationY:null}; if(playerModel&&playerModel!=='error'){ try{const dS=0.8;//<<< ADJUST SCALE const mI=playerModel.clone();mI.scale.set(dS,dS,dS);mI.traverse(function(c){if(c.isMesh)c.castShadow=true;}); const vY=pD.y;mI.position.set(pD.x,vY,pD.z);mI.rotation.y=pD.rotationY;scene.add(mI);players[pD.id].mesh=mI;players[pD.id].targetPosition=mI.position.clone();players[pD.id].targetRotationY=mI.rotation.y;}catch(e){addPlayerFallbackMesh(pD);}}else{addPlayerFallbackMesh(pD);}}
function addPlayerFallbackMesh(pD) { if(!players[pD.id]||players[pD.id].mesh)return; console.warn(`Fallback for ${pD.id}`); try{const g=new THREE.CylinderGeometry(PLAYER_RADIUS,PLAYER_RADIUS,PLAYER_HEIGHT,8);const m=new THREE.MeshStandardMaterial({color:0xff00ff});const h=new THREE.Mesh(g,m); h.castShadow=true;const vY=pD.y+(PLAYER_HEIGHT/2); h.position.set(pD.x,vY,pD.z); h.rotation.y=pD.rotationY; scene.add(h); players[pD.id].mesh=h; players[pD.id].targetPosition=h.position.clone(); players[pD.id].targetRotationY=h.rotation.y;}catch(e){console.error(`Fallback error ${pD.id}:`,e);}}
function removePlayerMesh(pId) { if(players[pId]?.mesh){ try{ scene.remove(players[pId].mesh); if(players[pId].mesh.geometry)players[pId].mesh.geometry.dispose(); if(players[pId].mesh.material){if(Array.isArray(players[pId].mesh.material)){players[pId].mesh.material.forEach(function(m){m.dispose();});}else{players[pId].mesh.material.dispose();}} }catch(e){} players[pId].mesh=null; }}
function updateRemotePlayerPosition(pD) { if(pD.id!==localPlayerId&&players[pD.id]){ const p=players[pD.id];let vY;if(p.mesh&&p.mesh.geometry instanceof THREE.CylinderGeometry){vY=pD.y+(PLAYER_HEIGHT/2);}else{vY=pD.y;}p.targetPosition=new THREE.Vector3(pD.x,vY,pD.z);p.targetRotationY=pD.rotationY;p.x=pD.x;p.y=pD.y;p.z=pD.z;p.rotationY=pD.rotationY;p.name=pD.name;p.phrase=pD.phrase;}}

// --- Game Logic ---
function updatePlayer(dT) { if(gameState!=='playing'||!controls?.isLocked||!localPlayerId||!players[localPlayerId])return; const o=controls.getObject();const s=players[localPlayerId]; if(!s||s.health<=0)return; const sp=keys['ShiftLeft']?MOVEMENT_SPEED_SPRINTING:MOVEMENT_SPEED; const dS=sp*dT; const pP=o.position.clone(); velocityY-=GRAVITY*dT; o.position.y+=velocityY*dT; if(keys['KeyW']){controls.moveForward(dS);} if(keys['KeyS']){controls.moveForward(-dS);} if(keys['KeyA']){controls.moveRight(-dS);} if(keys['KeyD']){controls.moveRight(dS);} const cP=o.position; for(const id in players){if(id!==localPlayerId&&players[id].mesh&&players[id].mesh.visible){const oM=players[id].mesh; const dXZ=new THREE.Vector2(cP.x-oM.position.x,cP.z-oM.position.z).length(); if(dXZ<PLAYER_COLLISION_RADIUS*2){o.position.x=pP.x; o.position.z=pP.z; o.position.y=cP.y; break;}}} let gY=0; if(o.position.y<gY+PLAYER_HEIGHT){o.position.y=gY+PLAYER_HEIGHT;if(velocityY<0)velocityY=0;isOnGround=true;}else{isOnGround=false;} if(o.position.y<VOID_Y_LEVEL&&s.health>0){socket.emit('fellIntoVoid');s.health=0;updateHealthBar(0);showKillMessage("You fell into the void.");} updateViewModel(dT); const lP=o.position.clone(); lP.y-=PLAYER_HEIGHT; const lS=players[localPlayerId]; const pc=lP.distanceToSquared(new THREE.Vector3(lS?.x??0,lS?.y??0,lS?.z??0))>0.001; const cR=new THREE.Euler().setFromQuaternion(camera.quaternion,'YXZ'); const cRY=cR.y; const rc=Math.abs(cRY-(lS?.rotationY??0))>0.01; if(pc||rc){ if(lS){lS.x=lP.x;lS.y=lP.y;lS.z=lP.z;lS.rotationY=cRY;} socket.emit('playerUpdate',{x:lP.x,y:lP.y,z:lP.z,rotationY:cRY});}}
function updateViewModel(dT) { if(!gunViewModel||!camera)return; currentRecoilOffset.lerp(new THREE.Vector3(0,0,0),dT*RECOIL_RECOVER_SPEED); const fP=GUN_POS_OFFSET.clone().add(currentRecoilOffset); gunViewModel.position.copy(fP); gunViewModel.rotation.copy(camera.rotation); }
...
function shoot() { if(gameState!=='playing'||!socket||!localPlayerId||!controls?.isLocked||!players[localPlayerId]||players[localPlayerId].health<=0)return; currentRecoilOffset.copy(RECOIL_AMOUNT); if(gunshotSound){try{gunshotSound.cloneNode().play().catch(function(e){});}catch(e){}} const bP=new THREE.Vector3(),bD=new THREE.Vector3(); if(!camera)return; if(gunViewModel){const mO=new THREE.Vector3(0,-0.05,-0.5).applyQuaternion(camera.quaternion);bP.copy(camera.position).add(mO);}else{camera.getWorldPosition(bP);} camera.getWorldDirection(bD); socket.emit('shoot',{position:{x:bP.x,y:bP.y,z:bP.z},direction:{x:bD.x,y:bD.y,z:bD.z}});}
...
function init() {
    ...
    try { controls=new THREE.PointerLockControls(camera,document.body); controls.addEventListener('lock',function(){console.log('Pointer Locked');}); controls.addEventListener('unlock',function(){console.log('Pointer Unlocked'); /* No state change */}); console.log("Controls initialized."); } catch (e) { console.error("CRITICAL Controls Init Error:", e); setGameState('loading',{message: "FATAL: Controls Error!", error: true}); return; }
