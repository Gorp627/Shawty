...
function updatePlayer(deltaTime) { if(gameState!=='playing'||!controls?.isLocked||!localPlayerId||!players[localPlayerId])return; const playerObject=controls.getObject(); const playerState=players[localPlayerId]; if(!playerState||playerState.health<=0)return; const currentSpeed=keys['ShiftLeft']?MOVEMENT_SPEED_SPRINTING:MOVEMENT_SPEED; const speed=currentSpeed*deltaTime; const previousPosition=playerObject.position.clone(); velocityY-=GRAVITY*deltaTime; playerObject.position.y+=velocityY*deltaTime; if(keys['KeyW']){controls.moveForward(speed);} if(keys['KeyS']){controls.moveForward(-speed);} if(keys['KeyA']){controls.moveRight(-speed);} if(keys['KeyD']){controls.moveRight(speed);} const currentPosition=playerObject.position; for(const id in players){if(id!==localPlayerId&&players[id].mesh&&players[id].mesh.visible){const otherMesh=players[id].mesh; const dXZ=new THREE.Vector2(currentPosition.x-otherMesh.position.x,currentPosition.z-otherMesh.position.z).length(); if(dXZ<PLAYER_COLLISION_RADIUS*2){ /* console.log("Player collision revert"); */ playerObject.position.x=previousPosition.x; playerObject.position.z=previousPosition.z; playerObject.position.y=currentPosition.y; break;}}} let groundY=0; if(playerObject.position.y<groundY+PLAYER_HEIGHT){playerObject.position.y=groundY+PLAYER_HEIGHT;if(velocityY<0)velocityY=0;isOnGround=true;}else{isOnGround=false;} if(playerObject.position.y<VOID_Y_LEVEL&&playerState.health>0){console.log("Void death");socket.emit('fellIntoVoid');playerState.health=0;updateHealthBar(0);showKillMessage("You fell into the void.");} updateViewModel(deltaTime); const logicalPos=playerObject.position.clone(); logicalPos.y-=PLAYER_HEIGHT; const lastSent=players[localPlayerId]; const posChanged=logicalPos.distanceToSquared(new THREE.Vector3(lastSent?.x??0,lastSent?.y??0,lastSent?.z??0))>0.001; const camRot=new THREE.Euler().setFromQuaternion(camera.quaternion,'YXZ'); const curRotY=camRot.y; const rotChanged=Math.abs(curRotY-(lastSent?.rotationY??0))>0.01; if(posChanged||rotChanged){ if(lastSent){lastSent.x=logicalPos.x;lastSent.y=logicalPos.y;lastSent.z=logicalPos.z;lastSent.rotationY=curRotY;} socket.emit('playerUpdate',{x:logicalPos.x,y:logicalPos.y,z:logicalPos.z,rotationY:curRotY});}}
...
function shoot() { /* console.log("Shoot func start.");*/ if(gameState!=='playing'||!socket||!localPlayerId||!controls?.isLocked||!players[localPlayerId]||players[localPlayerId].health<=0){return;} currentRecoilOffset.copy(RECOIL_AMOUNT); /* console.log("Recoil applied.");*/ if(gunshotSound){try{gunshotSound.cloneNode().play().catch(e=>console.warn("Sound fail:",e));/* console.log("Gun sound played.");*/}catch(e){console.error("Sound error:",e);}}else console.warn("No gunshot sound."); const bulPos=new THREE.Vector3(),bulDir=new THREE.Vector3(); if(!camera){console.error("! Camera missing");return;} if(gunViewModel){const muzzleOffset=new THREE.Vector3(0,-0.05,-0.5).applyQuaternion(camera.quaternion);bulPos.copy(camera.position).add(muzzleOffset);/* console.log("Bullet origin: gun approx");*/}else{camera.getWorldPosition(bulPos);/* console.log("Bullet origin: cam");*/} camera.getWorldDirection(bulDir); /* console.log("Emit shoot event.");*/ socket.emit('shoot',{position:{x:bulPos.x,y:bulPos.y,z:bulPos.z},direction:{x:bulDir.x,y:bulDir.y,z:bulDir.z}}); /* console.log("Shoot emitted.");*/ }
