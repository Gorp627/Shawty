...
function setGameState(newState, options = {}) {
    ...
    switch(newState){case'loading':if(loadingScreen){loadingScreen.style.display='flex';const p=loadingScreen.querySelector('p');if(p)p.innerHTML=options.message||'Loading...';if(options.assets)loadingScreen.classList.add('assets');if(options.error&&p){p.style.color='#e74c3c';loadingScreen.classList.add('error');}}break;case'homescreen':if(homeScreen){homeScreen.style.display='flex';requestAnimationFrame(()=>{homeScreen.classList.add('visible');});playerCountSpan=playerCountSpan||document.getElementById('playerCount');if(playerCountSpan)playerCountSpan.textContent=options.playerCount??playerCountSpan.textContent??'?';if(controls?.isLocked)controls.unlock();const obj=scene?.getObjectByName("PlayerControls");if(obj)scene.remove(obj);removeGunViewModel();joinButton=joinButton||document.getElementById('joinButton');if(joinButton){joinButton.disabled=false;joinButton.textContent="Join Game";}}break;case'joining':joinButton=joinButton||document.getElementById('joinButton');if(joinButton){joinButton.disabled=true;joinButton.textContent="Joining...";}if(options.waitingForAssets)setGameState('loading',{message:"Loading Assets...",assets:true});break;case'playing':const cElem=document.getElementById('gameCanvas');if(gameUI){gameUI.style.display='block';requestAnimationFrame(()=>{gameUI.classList.add('visible');});}else console.error("! gameUI");if(cElem){cElem.style.display='block';}else console.error("! gameCanvas");if(scene&&controls){if(!scene.getObjectByName("PlayerControls")){controls.getObject().name="PlayerControls";scene.add(controls.getObject());}attachGunViewModel();setTimeout(()=>{if(gameState==='playing'&&!controls.isLocked)controls.lock();},100);}else console.error("! Scene/Controls missing!");onWindowResize();} console.log(`Switched state from ${previousState} to ${gameState}`);
}

function loadSound() { try { gunshotSound=new Audio(SOUND_PATH_GUNSHOT); gunshotSound.volume=0.4; gunshotSound.preload='auto'; gunshotSound.load(); } catch(e){ console.error("Audio error:",e); gunshotSound = null; } }
function loadPlayerModel() { playerModelLoadState = 'loading'; if (!loader) return; loader.load(PLAYER_MODEL_PATH,(gltf)=>{ playerModel=gltf.scene;playerModel.traverse((c)=>{if(c.isMesh)c.castShadow=true;});playerModelLoadState='loaded';checkAssetsReady(); },undefined,(err)=>{ console.error("Player model LOAD ERROR:",err);playerModelLoadState='error';checkAssetsReady(); }); }
...
function handleInitialize(data) { localPlayerId = data.id; for(const id in players)removePlayerMesh(id); players = {}; bullets = []; let iPosX=0,iPosY=0,iPosZ=0; for(const id in data.players){ const pD=data.players[id]; if(id===localPlayerId){ players[id]={...pD,name:localPlayerName,phrase:localPlayerPhrase,mesh:null}; iPosX=pD.x;iPosY=pD.y;iPosZ=pD.z; const visY=iPosY+PLAYER_HEIGHT; if(controls?.getObject()){controls.getObject().position.set(iPosX,visY,iPosZ);}else console.error("Controls missing!"); velocityY=0;isOnGround=true; updateHealthBar(pD.health); infoDiv.textContent=`Playing as ${localPlayerName}`; }else{addPlayer(pD);}} console.log("Game initialized players:",Object.keys(players).length); setGameState('playing');}
...
function shoot() { if(gameState!=='playing'||!socket||!localPlayerId||!controls?.isLocked||!players[localPlayerId]||players[localPlayerId].health<=0)return; currentRecoilOffset.copy(RECOIL_AMOUNT); if(gunshotSound){try{gunshotSound.cloneNode().play().catch(e=>console.warn("Sound fail:",e));}catch(e){console.error("Sound error:",e);}} const bP=new THREE.Vector3(),bD=new THREE.Vector3(); if(!camera)return; if(gunViewModel){const mO=new THREE.Vector3(0,-0.05,-0.5).applyQuaternion(camera.quaternion);bP.copy(camera.position).add(mO);}else{camera.getWorldPosition(bP);} camera.getWorldDirection(bD); socket.emit('shoot',{position:{x:bP.x,y:bP.y,z:bP.z},direction:{x:bD.x,y:bD.y,z:bD.z}});}
function spawnBullet(bulletData) { const g=new THREE.SphereGeometry(0.1,6,6);const m=new THREE.MeshBasicMaterial({color:0xffff00});const h=new THREE.Mesh(g,m); h.position.set(bulletData.position.x,bulletData.position.y,bulletData.position.z); const v=new THREE.Vector3(bulletData.direction.x,bulletData.direction.y,bulletData.direction.z).normalize().multiplyScalar(BULLET_SPEED); bullets.push({id:bulletData.bulletId,mesh:h,velocity:v,ownerId:bulletData.shooterId,spawnTime:Date.now()}); scene.add(h); }
function updateBullets(deltaTime) { const rI=[]; for(let i=bullets.length-1;i>=0;i--){const b=bullets[i];if(!b?.mesh){if(!rI.includes(i))rI.push(i);continue;}b.mesh.position.addScaledVector(b.velocity,deltaTime);let h=false;for(const pId in players){if(pId!==b.ownerId&&players[pId].mesh&&players[pId].mesh.visible){const pM=players[pId].mesh; const pP=new THREE.Vector3();pM.getWorldPosition(pP);const d=b.mesh.position.distanceTo(pP);const pSR=(pM.scale?.x||1)*PLAYER_RADIUS; const t=pSR+0.1; if(d<t){h=true;if(b.ownerId===localPlayerId){socket.emit('hit',{targetId:pId,damage:10});}if(!rI.includes(i))rI.push(i);scene.remove(b.mesh);break;}}}if(h)continue; if(Date.now()-b.spawnTime>BULLET_LIFETIME){if(!rI.includes(i))rI.push(i);scene.remove(b.mesh);}} if(rI.length>0){ rI.sort((a,b)=>b-a); for(const idx of rI){ bullets.splice(idx,1); } } }
...
function handlePlayerJoined(playerData) { if(playerData.id!==localPlayerId&&!players[playerData.id]){addPlayer(playerData);}}
function handlePlayerLeft(playerId) { removePlayerMesh(playerId);delete players[playerId];}
function handleHealthUpdate(data) { if(players[data.id]){players[data.id].health=data.health;if(data.id===localPlayerId){updateHealthBar(data.health);}}}
...
function init() {
    ...
    try { scene=new THREE.Scene(); scene.background=new THREE.Color(0x87ceeb); scene.fog=new THREE.Fog(0x87ceeb,0,150); camera=new THREE.PerspectiveCamera(75,window.innerWidth/window.innerHeight,0.1,1000); renderer=new THREE.WebGLRenderer({canvas:canvas,antialias:true}); renderer.setSize(window.innerWidth,window.innerHeight); renderer.shadowMap.enabled=true; clock=new THREE.Clock(); console.log("Init THREE.GLTFLoader..."); loader=new THREE.GLTFLoader(); console.log("Init THREE.DRACOLoader..."); dracoLoader=new THREE.DRACOLoader(); dracoLoader.setDecoderPath('https://cdn.jsdelivr.net/npm/three@0.128.0/examples/js/libs/draco/'); dracoLoader.setDecoderConfig({type:'js'}); loader.setDRACOLoader(dracoLoader); console.log("Three.js core init."); } catch(e){ console.error("CRITICAL Three.js init error:", e); setGameState('loading',{message:"FATAL: Graphics Error!",error:true}); return; }
    try { const aL=new THREE.AmbientLight(0xffffff,0.6);scene.add(aL);const dL=new THREE.DirectionalLight(0xffffff,0.9);dL.position.set(10,15,10);dL.castShadow=true;dL.shadow.mapSize.width=1024;dL.shadow.mapSize.height=1024;scene.add(dL); } catch(e){ console.error("Lighting Error:", e); setGameState('loading',{message:"FATAL: Graphics Error(Light)!",error:true}); return; }
    try { controls=new THREE.PointerLockControls(camera,document.body); controls.addEventListener('lock',()=>{console.log('Pointer Locked');}); controls.addEventListener('unlock',()=>{console.log('Pointer Unlocked');}); console.log("Controls init."); } catch(e){ console.error("CRITICAL Controls Init Error:", e); setGameState('loading',{message:"FATAL: Controls Error!",error:true}); return; }
    console.log("Starting assets/socket..."); loadSound(); loadPlayerModel(); loadGunModel(); loadMap(MAP_PATH); setupSocketIO();
