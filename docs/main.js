...
function setGameState(newState, options = {}) {
    // console.log(`Setting game state to: ${newState}`, options); // Reduce log noise
...
// --- Asset Loading ---
function loadSound() {
     try { gunshotSound = new Audio(SOUND_PATH_GUNSHOT); gunshotSound.volume = 0.4; gunshotSound.preload = 'auto'; gunshotSound.load(); /* console.log("Gunshot sound obj created.");*/ } catch(e){ console.error("Audio error:",e); gunshotSound = null; }
}

function loadPlayerModel() {
    playerModelLoadState = 'loading'; /* console.log(`Loading player model: ${PLAYER_MODEL_PATH}`); */
    ...
    undefined, // Progress
    (error)=>{ // Error
        console.error("!!! Player model LOAD ERROR:",error); playerModelLoadState='error'; checkAssetsReady();
    });
}

function loadGunModel() {
    gunModelLoadState = 'loading'; /* console.log(`Loading gun model: ${GUN_MODEL_PATH}`); */
    ...
    (gltf)=>{ // Success
        ...
        gunModel=gltf.scene; gunModel.traverse((c)=>{if(c.isMesh){c.castShadow=false; c.receiveShadow=false;}}); gunModelLoadState='loaded'; checkAssetsReady();
    },
    undefined, // Progress
    (error)=>{ // Error
        console.error("!!! Gun model LOAD ERROR:",error); gunModelLoadState='error'; checkAssetsReady();
    });
}

function loadMap(mapPath) {
    mapLoadState = 'loading'; /* console.log(`Loading map: ${mapPath}`); */
    ...
    undefined, // Progress
    (error)=>{ // Error
        console.error(`!!! Map LOAD ERROR (${mapPath}):`,error); mapLoadState='error'; checkAssetsReady();
    });
}


function checkAssetsReady() {
    // console.log(`checkReady: Map=${mapLoadState}, PModel=${playerModelLoadState}, GModel=${gunModelLoadState}`); // Reduce noise
...
function setupSocketIO() {
    ...
    socket.on('connect_error',(err)=>{ console.error('Connection Error:',err.message); mapLoadState='error'; playerModelLoadState='error'; gunModelLoadState = 'error'; assetsReady=false; setGameState('loading',{message:`Connection Failed!<br/>${err.message}`,error:true}); });
    ...
    socket.on('initialize',(data)=>{ handleInitialize(data); }); // Use separate handler
...
function addPlayer(playerData) {
    // console.log(`Adding player ${playerData.id} (${playerData.name})`); // Reduce noise
    ...
    if(playerModel&&playerModel!=='error'){
        try { // Added try-catch for cloning/adding
            const modelInstance=playerModel.clone();
            const desiredScale=0.8;// <<<=== ADJUST THIS SCALE (e.g., 0.5, 1.0, 1.2)
            modelInstance.scale.set(desiredScale,desiredScale,desiredScale);
            modelInstance.traverse((c)=>{if(c.isMesh)c.castShadow=true;});
            ...
            players[playerData.id].targetRotationY=modelInstance.rotation.y;
             ...
             console.error(`Model clone/add error for ${playerData.id}:`,e);
             addPlayerFallbackMesh(playerData);
        }
    } else{
        console.warn(`Fallback for ${playerData.id}`);addPlayerFallbackMesh(playerData);
    }
}
function addPlayerFallbackMesh(playerData) {
    if(!players[playerData.id]||players[playerData.id].mesh)return;
    // console.warn(`Using fallback for ${playerData.id}`); // Reduce noise
    try { // Added try-catch
        ...
        mesh.castShadow=true;const visY=playerData.y+(PLAYER_HEIGHT/2); mesh.position.set(playerData.x,visY,playerData.z); mesh.rotation.y=playerData.rotationY; scene.add(mesh);
        players[playerData.id].mesh=mesh; players[playerData.id].targetPosition=mesh.position.clone(); players[playerData.id].targetRotationY=mesh.rotation.y;
    } catch(e){ console.error(`Fallback mesh error ${playerData.id}:`,e);}
}
...
function updatePlayer(deltaTime) {
    ...
    if(playerObject.position.y<groundY+PLAYER_HEIGHT){playerObject.position.y=groundY+PLAYER_HEIGHT;if(velocityY<0)velocityY=0;isOnGround=true;}else{isOnGround=false;}
...
function attachGunViewModel() {
     if(!gunModel||gunModel==='error'||!camera){console.error("! Gun template/camera missing");return;} if(gunViewModel)return;
     gunViewModel=gunModel.clone(); gunViewModel.scale.set(GUN_SCALE,GUN_SCALE,GUN_SCALE); gunViewModel.position.copy(GUN_POS_OFFSET);
     camera.add(gunViewModel); // Attach to camera
...
function shoot() {
    // console.log("Shoot func start."); // Reduce noise
    ...
    // Decide bullet origin: Visually near gun, aimed from camera center
    if(gunViewModel){ const muzzleOffset=new THREE.Vector3(0,-0.05,-0.5).applyQuaternion(camera.quaternion); bulPos.copy(camera.position).add(muzzleOffset); }
    else { camera.getWorldPosition(bulPos); }
    camera.getWorldDirection(bulDir); // Use camera direction for aiming
    socket.emit('shoot',{position:{x:bulPos.x,y:bulPos.y,z:bulPos.z},direction:{x:bulDir.x,y:bulDir.y,z:bulDir.z}}); // Send to server
}

function spawnBullet(bulletData) {
    // console.log(`Spawning bullet ${bulletData.bulletId}`); // Reduce noise
    const geo=new THREE.SphereGeometry(0.1,6,6); const mat=new THREE.MeshBasicMaterial({color:0xffff00}); const mesh=new THREE.Mesh(geo,mat);
    mesh.position.set(bulletData.position.x,bulletData.position.y,bulletData.position.z);
    const vel=new THREE.Vector3(bulletData.direction.x,bulletData.direction.y,bulletData.direction.z).normalize().multiplyScalar(BULLET_SPEED);
    bullets.push({id:bulletData.bulletId,mesh:mesh,velocity:vel,ownerId:bulletData.shooterId,spawnTime:Date.now()});
    scene.add(mesh);
}
function updateBullets(deltaTime) {
    const removeIdx=[]; for(let i=bullets.length-1;i>=0;i--){const b=bullets[i];if(!b?.mesh){if(!removeIdx.includes(i))removeIdx.push(i);continue;}b.mesh.position.addScaledVector(b.velocity,deltaTime);let hit=false;for(const pId in players){if(pId!==b.ownerId&&players[pId].mesh&&players[pId].mesh.visible){const pMesh=players[pId].mesh; const pPos=new THREE.Vector3();pMesh.getWorldPosition(pPos);const dist=b.mesh.position.distanceTo(pPos);const pScaleR=(pMesh.scale?.x||1)*PLAYER_RADIUS; const thresh=pScaleR+0.1; if(dist<thresh){/* console.log(`Client hit: Bul ${b.id} -> P ${pId}`); */ hit=true;if(b.ownerId===localPlayerId){socket.emit('hit',{targetId:pId,damage:10});}if(!removeIdx.includes(i))removeIdx.push(i);scene.remove(b.mesh);break;}}}if(hit)continue; if(Date.now()-b.spawnTime>BULLET_LIFETIME){if(!removeIdx.includes(i))removeIdx.push(i);scene.remove(b.mesh);}}
    if(removeIdx.length>0){ removeIdx.sort((a,b)=>b-a); for(const idx of removeIdx){ if(bullets[idx]?.mesh){} bullets.splice(idx,1); } }
}
...
function handlePlayerDied(data) { /* console.log(`Died handled: ${data.targetId}`);*/ if(players[data.targetId]){players[data.targetId].health=0;if(players[data.targetId].mesh)players[data.targetId].mesh.visible=false;}if(data.targetId===localPlayerId){updateHealthBar(0);const k N=data.killerName||'environment';const kP=data.killerPhrase||'...';let msg=`You just got ${kP} by ${kN}.`;if(!data.killerId)msg=`You died.`;showKillMessage(msg);infoDiv.textContent=`YOU DIED`;}}
