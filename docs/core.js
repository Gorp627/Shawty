...
function init() {
    ...
    if (!loadingScreen || !homeScreen || !gameUI || !canvas || !joinButton || !playerNameInput || !playerPhraseInput || !homeScreenError || !infoDiv || !healthBarFill || !healthText || !killMessageDiv ) {
         console.error("! Critical UI element missing! Aborting init.");
         if (loadingScreen && loadingScreen.style) { loadingScreen.style.display = 'flex'; const p = loadingScreen.querySelector('p'); if (p) p.innerHTML = "FATAL: UI Init Error!"; p.style.color = 'red'; }
         return; // Stop init
    }
...
// ========================================================
// Call init directly after DOM is ready
...
console.log("core.js loaded and executed."); // Changed log slightly
