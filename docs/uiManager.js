...
const UIManager = {
     // References (will be populated by initialize)
     ...
     // Get references ONCE after DOM load (called by game.js init)
     initialize: function() {
         ...
         if (!this.loadingScreen || !this.homeScreen || !this.gameUI || !this.canvas || !this.joinButton ) {
             ...
             // Maybe display error differently if possible?
     ...
     bindStateListeners: function(stateMachine) {
         if (!stateMachine || typeof stateMachine.on !== 'function') {
             console.error("UIManager: Invalid stateMachine provided for binding.");
             return;
         }
     ...
     showHomescreen: function(playerCount = '?') { // Provide default for playerCount
          ...
          requestAnimationFrame(() => { this.homeScreen?.classList.add('visible'); }); // Add null check
     },
     showJoining: function(waitingAssets = false) {
         if (waitingAssets) {
              ...
              if(this.joinButton) { this.joinButton.disabled = true; this.joinButton.textContent = "Joining..."; }
              if(this.homeScreen && this.homeScreen.style.display === 'none') { this.showLoading("Joining..."); } // Show loading if home hidden
         }
     },
     showGame: function() {
         ...
         requestAnimationFrame(() => { this.gameUI?.classList.add('visible'); this.canvas?.classList.add('visible'); }); // Add null checks
