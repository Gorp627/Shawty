...
const Network = {
    ...
    setupSocketIO: function() {
        ...
        // Event Listeners
        socket.on('connect', () => { console.log('Socket Connected! ID:', socket.id); networkIsInitialized = true; currentGameInstance?.attemptProceedToGame(); }); // Check if ready on connect
    ...
    // Handlers
    _getPlayer: function(id) { return players?.[id] || null; }, // Add check for players existence
    ...
    _removePlayer: function(playerId) { // Removes visual AND physics body
        ...
        if (player || bodyHandle !== undefined) {
            ...
            if (currentGameInstance?.playerRigidBodies) delete currentGameInstance.playerRigidBodies[playerId];
        }
    },
    ...
    handleInitialize: function(data) {
         ...
         initializationData = data; networkIsInitialized = true;
         console.log("Network Initialized flag set TRUE.");
         // Let game instance check all readiness flags
        ...
        if (playerData?.id !== localPlayerId && !this._getPlayer(playerData.id)) {
            ...
            if (newPlayer instanceof ClientPlayer && RAPIER && rapierWorld && currentGameInstance) { // Check deps
                 try {
                     ...
                     const playerColliderDesc = RAPIER.ColliderDesc.capsule(capsuleHalfHeight, playerRadius).setFriction(0.5).setRestitution(0.1).setActiveEvents(RAPIER.ActiveEvents.COLLISION_EVENTS);
                     // --- CORRECTED ROTATION ---
                     const rotY = playerData.rotationY || 0;
                     const q = RAPIER.Quaternion.fromAxisAngle({ x: 0, y: 1, z: 0 }, rotY);
                     // --- END CORRECTION ---
                     const rigidBodyDesc = RAPIER.RigidBodyDesc.kinematicPositionBased().setTranslation(playerData.x, bodyCenterY, playerData.z).setRotation(q); // Use calculated quat
                     const body = rapierWorld.createRigidBody(rigidBodyDesc); if (!body) throw new Error("Joined player body fail.");
                     const collider = rapierWorld.createCollider(playerColliderDesc, body.handle); currentGameInstance.playerRigidBodies[playerData.id] = body.handle;
    ...
    handleGameStateUpdate: function(state) { // Updates remote kinematic bodies
        ...
        for (const id in state.players) { const sPD = state.players[id]; if (id !== localPlayerId) {
             const rbHandle = currentGameInstance.physicsBodies[id]; const rb = rbHandle !== undefined ? rapierWorld.getRigidBody(rbHandle) : null; const rp = players[id];
             if (rb) { // Update kinematic body
                 const playerHeight = CONFIG?.PLAYER_HEIGHT || 1.8; const bodyCenterY = sPD.y + playerHeight / 2.0;
                 rb.setNextKinematicTranslation({ x: sPD.x, y: bodyCenterY, z: sPD.z }, true); // Smooth translation
                 // --- CORRECTED ROTATION ---
                 const rotY = sPD.r || 0; const q = RAPIER.Quaternion.fromAxisAngle({ x: 0, y: 1, z: 0 }, rotY);
                 rb.setNextKinematicRotation(q, true); // Smooth rotation
                 // --- END CORRECTION ---
...
console.log("network.js loaded (Rapier Integration - Fixed Remote Rotation)");
