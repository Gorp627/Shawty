...
// Needs access to constants like PLAYER_HEIGHT, SERVER_URL from CONFIG
...
const Network = {
    ...
    handleShotFired: function(d){ if(typeof spawnBullet === 'function') spawnBullet(d); },
     ...
     attemptJoinGame: function() { if(!playerNameInput||!playerPhraseInput||!homeScreenError)getUIElements(); localPlayerName = playerNameInput?.value.trim() || 'Anonymous'; localPlayerPhrase = playerPhraseInput?.value.trim() || '...'; if(!localPlayerName){if(homeScreenError)homeScreenError.textContent='Enter name';return;} if(localPlayerPhrase.length>20){if(homeScreenError)homeScreenError.textContent='Phrase too long';return;} if(homeScreenError)homeScreenError.textContent=''; console.log(`Joining as "${localPlayerName}" | Assets: ${assetsReady}`); if(typeof stateMachine!=='undefined') stateMachine.transitionTo('joining',{waitingForAssets:!assetsReady}); if(assetsReady){ this.sendJoinDetails(); }else console.log("Wait assets..."); },
     sendJoinDetails: function() { console.log("--- sendJoinDetails called ---"); if(socket?.connected&&stateMachine.is('joining')){ console.log("Sending details..."); socket.emit('setPlayerDetails',{name:localPlayerName,phrase:localPlayerPhrase}); } else if (!stateMachine.is('joining')){ console.warn("Not joining state."); if(typeof stateMachine!=='undefined') stateMachine.transitionTo('homescreen',{playerCount:playerCountSpan?.textContent??'?'});} else{ console.error("Socket disconnected."); if(homeScreenError)homeScreenError.textContent='Connection lost.'; if(typeof stateMachine!=='undefined') stateMachine.transitionTo('homescreen',{playerCount:playerCountSpan?.textContent??'?'});}},
