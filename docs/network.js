// docs/network.js (Adapted for Rapier - Euler Rotation Fix)
...
const Network = {
        ...
        if (playerData?.id !== localPlayerId && !this._getPlayer(playerData.id)) {
            ...
            if (newPlayer instanceof ClientPlayer && RAPIER && rapierWorld && currentGameInstance) { // Check deps
                 try {
                     ...
                     // ---<<< CORRECTED ROTATION: Set on RigidBodyDesc >>>---
    ...
    handleGameStateUpdate: function(state) { // Updates remote kinematic bodies
        ...
        for (const id in state.players) { const sPD = state.players[id]; if (id !== localPlayerId) {
             const rbHandle = currentGameInstance.playerRigidBodyHandles[id];
             ...
             if (rb) { // Update kinematic body
                 ...
                 const rotY = sPD.r || 0;
                 rb.setNextKinematicRotation({ x: 0, y: rotY, z: 0 }, true); // Update with Euler object
                 // --- END CORRECTION ---
