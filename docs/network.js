...
const Network = {
    ...
    // Handlers
    _getPlayer: function(id) { return typeof players !== 'undefined' ? (players[id] || null) : null; }, // Add players check
    ...
    handleInitialize: function(data) {
         ...
         if(typeof currentGameInstance !== 'undefined' && currentGameInstance?.attemptProceedToGame) {currentGameInstance.attemptProceedToGame();} else { console.error("Missing attemptProceedToGame on game instance");}
    },
    ...
    handlePlayerLeft: function(playerId) {
         if(playerId){ const pName=players?.[playerId]?.name||'Player'; console.log(`Player left event: ${pName}`); this._removePlayer(playerId); if(UIManager?.showKillMessage) UIManager.showKillMessage(`${pName} left.`);}
    },
    ...
    handlePlayerRespawned: function(playerData) {
         ...
         if (playerData.id === localPlayerId) {
              ...
              if (!player){ player={isLocal: true}; players[localPlayerId]=player; } // Create cache if missing
              ...
              // --- Respawn Raycast Logic (Requires global mapMesh and THREE) ---
              if (typeof mapMesh !== 'undefined' && mapMesh && typeof THREE !== 'undefined') { // Check dependencies
                    if (!raycaster) raycaster = new THREE.Raycaster(); // Ensure global raycaster exists
                    ...
                    while (!foundGround && attempts < maxSpawnAttempts) {
                        ...
                        const intersects = raycaster.intersectObject(mapMesh, true); // Use global mapMesh
              ...
              } else { console.error("mapMesh or THREE not available during respawn! Defaulting to Y=0."); foundGroundY = 0; }
              // --- End Raycast Logic ---
